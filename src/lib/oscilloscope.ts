 ...
 */
export function updateYMax(prev: number, target: number, floor: number): number {
...
export function drawScopeFrame(
  ...
  ctx.moveTo(p.left, yHalfUp); ctx.lineTo(W - p.right, yHalfUp)
  ctx.moveTo(p.left, yHalfDn); ctx.lineTo(W - p.right, yHalfDn)
  ctx.stroke()
