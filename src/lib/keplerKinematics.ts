...
export const DEFAULT_DV_C = 0.660   // km/s  (= 660 m/s)
 ...
 */
export function computeMission(dvA: number, dvB: number, dvC: number): MissionData {
  ...
  const nuMax    = hyperbola.e > 1
    ? Math.acos(Math.max(-1, -1 / hyperbola.e))
    : Math.PI * 0.9

  return { vCirc, orbit0, vAplus, orbit1, vB1, vB2, orbit2, vCbefore, vFinal, hyperbola, nuMax }
}

/** World position (km) at true anomaly ν. Periapsis is always on the +x axis. */
export function orbitPosition(orbit: OrbitData, nu: number): { x: number; y: number } {
...
export function orbitVelocity(orbit: OrbitData, nu: number): { vx: number; vy: number } {
