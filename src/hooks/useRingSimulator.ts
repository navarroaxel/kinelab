...
export function useRingSimulator() {
  ...
  const [visibility, setVisibility] =
    useState<RingVisibility>(INITIAL_VISIBILITY);
  const [paused, setPaused] = useState(false);

  // RK4 integration state — refs so frames don't trigger re-renders
  const thetaRef = useRef(0);
  const thetaDotRef = useRef(
    speedToThetaDot(INITIAL_PARAMS.initialSpeed, INITIAL_PARAMS.radius),
  );
  ...
  const subscribeTick = useCallback((fn: () => void) => {
    ...
    return () => {
      tickListenersRef.current.delete(fn);
    };
  }, []);
