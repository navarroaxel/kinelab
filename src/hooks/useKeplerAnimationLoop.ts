...
function renderKepler(
  ...
  if (visibility.showOrbits) {
    drawOrbitCurve(ctx, m.orbit0,    sc, dark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.10)', 0, 2 * Math.PI, false)
    drawOrbitCurve(ctx, m.orbit1,    sc, dark ? 'rgba(100,160,245,0.55)' : 'rgba(59,139,212,0.65)', 0, Math.PI, false)
    drawOrbitCurve(ctx, m.orbit2,    sc, dark ? 'rgba(175,115,240,0.55)' : 'rgba(120,60,185,0.65)', Math.PI, 2 * Math.PI, false)
    if (phase === 3) {
      drawOrbitCurve(ctx, m.hyperbola, sc, dark ? 'rgba(250,185,55,0.65)' : 'rgba(215,145,30,0.75)', 0, m.nuMax * 0.9, true)
    }
  }
  ...
  // Atmosphere glow
  const grd = ctx.createRadialGradient(mars.x, mars.y, marsR * 0.8, mars.x, mars.y, marsR * 1.35)
  ...
  ctx.restore()
  // Planet body
  drawDot(ctx, mars.x, mars.y, Math.max(marsR, 6),
    dark ? '#D06040' : '#CD5C3A',
    dark ? '#8B3020' : '#8B3A20')
  drawLabel(ctx, 'Mars', mars.x - marsR - 22, mars.y, dark ? '#D0A090' : '#8B3A20')
  ...
  if (visibility.showVelocity) {
    ...
    if (vMag > 0) {
      ...
      drawArrow(ctx, spSc.x, spSc.y, endX, endY,
        dark ? '#5DCAA5' : '#1D9E75', 2)
