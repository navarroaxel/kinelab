...
function renderPinSlot(
  ...
  if (visibility.showAngles) {
    ...
    drawAngleArc(ctx, aSc.x, aSc.y, params.r * scale * 0.45, 0, phiScreen, colors.radialVelocity, 'Φ')

    // Theta arc at O: from screen 0 (right) to direction O→B
    const thetaScreen = Math.atan2(bSc.y - oSc.y, bSc.x - oSc.x)
    drawAngleArc(ctx, oSc.x, oSc.y, params.d * scale * 0.28, 0, thetaScreen, colors.acceleration, 'θ')
  }
  ...
  if (visibility.showVPerp) {
    ...
    drawArrow(ctx, bSc.x, bSc.y, perpEnd.x, perpEnd.y, colors.transverseVelocity, 2)
    drawLabel(ctx, 'V⊥', perpEnd.x + 6, perpEnd.y - 6, colors.transverseVelocity)
  }
