...
export function useKeplerSimulator() {
  ...
  const [visibility, setVisibility] =
    useState<KeplerVisibility>(INITIAL_VISIBILITY);
  const [paused, setPaused] = useState(false);
  const [resetCount, setResetCount] = useState(0);
  const [metrics, setMetrics] = useState<KeplerMetrics>(() =>
    makeInitialMetrics(INITIAL_PARAMS),
  );
  ...
  const missionRef = useRef(
    computeMission(INITIAL_PARAMS.dvA, INITIAL_PARAMS.dvB, INITIAL_PARAMS.dvC),
  );
