...
function LegendItem({ color, label, dashed }: { color: string; label: string; dashed?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <div className="flex items-center w-10 justify-center flex-shrink-0">
        {dashed ? (
          ...
          <div className="w-5 h-2 rounded-sm" style={{ backgroundColor: color }} />
        )}
      </div>
...
export const KeplerLegend = memo(function KeplerLegend() {
  const { t } = useLanguage()
  return (
    <section className="rounded-lg border border-gray-200 dark:border-gray-700 p-3 flex flex-col gap-1.5">
      ...
      <LegendItem color="rgba(0,0,0,0.25)" label={t('kepler.legend.orbit0')} dashed />
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center w-10 justify-center flex-shrink-0">
          <div className="w-6 h-4 rounded-sm opacity-60" style={{ backgroundColor: 'rgba(0,175,140,0.5)' }} />
        </div>
        <span className="text-[11px] text-gray-700 dark:text-gray-300">{t('kepler.legend.area_sweep')}</span>
      </div>
    </section>
  )
})
