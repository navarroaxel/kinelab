...
interface Props {
  ...
  onSetParam: <K extends keyof SimulatorParams>(
    key: K,
    ...
    <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
      ...
      <section>
        <h3 className="mb-2 text-[10px] font-semibold tracking-wide text-gray-400 uppercase">
          {t("controls.section.geometry")}
        </h3>
        <div className="flex flex-col gap-3">
          <Slider
            label={t("controls.slider.pole_x")}
            id="poleX"
            min={-120}
            max={120}
            step={1}
            value={params.poleX}
            onChange={(v) => onSetParam("poleX", v)}
          />
          <Slider
            label={t("controls.slider.pole_y")}
            id="poleY"
            min={-120}
            max={120}
            step={1}
            value={params.poleY}
            onChange={(v) => onSetParam("poleY", v)}
          />
          <Slider
            label={t("controls.slider.radius")}
            id="circleRadius"
            min={40}
            max={140}
            step={5}
            value={params.circleRadius}
            onChange={(v) => onSetParam("circleRadius", v)}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          <Slider
            ...
            onChange={(v) => onSetParam("angularVelocity", v)}
          />
          <Slider
            label="α (°/s²)"
            ...
            max={100}
            step={5}
            value={params.angularAcceleration}
            onChange={(v) => onSetParam("angularAcceleration", v)}
          />
        </div>
      </section>
      ...
      <section>
        <h3 className="mb-2 text-[10px] font-semibold tracking-wide text-gray-400 uppercase">
          {t("controls.section.visibility")}
        </h3>
        <div className="flex flex-col gap-2">
          <Toggle
            id="showVelocity"
            label={t("controls.toggle.polar_velocity")}
            checked={visibility.showVelocity}
            onChange={() => onToggle("showVelocity")}
          />
          <Toggle
            id="showCartesian"
            label={t("controls.toggle.cartesian")}
            checked={visibility.showCartesian}
            onChange={() => onToggle("showCartesian")}
          />
          <Toggle
            id="showRVector"
            label={t("controls.toggle.r_vector")}
            checked={visibility.showRVector}
            onChange={() => onToggle("showRVector")}
          />
          <Toggle
            id="showAcceleration"
            label={t("controls.toggle.polar_accel")}
            checked={visibility.showAcceleration}
            onChange={() => onToggle("showAcceleration")}
          />
          <Toggle
            id="showNormalAccel"
            label={t("controls.toggle.normal_accel")}
            checked={visibility.showNormalAccel}
            onChange={() => onToggle("showNormalAccel")}
          />
          <Toggle
            id="showTrace"
            label={t("controls.toggle.trace")}
