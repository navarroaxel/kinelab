...
import type { QuickReturnParams, QuickReturnVisibility } from '@/types/simulator'

interface Props {
  params: QuickReturnParams
  visibility: QuickReturnVisibility
  onSetParam: <K extends keyof QuickReturnParams>(key: K, value: QuickReturnParams[K]) => void
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-3 bg-white dark:bg-gray-900 flex flex-col gap-4">
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          <Slider label={t('quick-return.controls.slider.r')}  id="qr-r"  min={20} max={100} step={5} value={params.r}  unit="u" onChange={v => onSetParam('r',  v)} />
          <Slider label={t('quick-return.controls.slider.L2')} id="qr-L2" min={30} max={180} step={5} value={params.L2} unit="u" onChange={v => onSetParam('L2', v)} />
          <Slider label={t('quick-return.controls.slider.L1')} id="qr-L1" min={80} max={320} step={5} value={params.L1} unit="u" onChange={v => onSetParam('L1', v)} />
        </div>
        {!constraintOk && (
      ...
      <section>
        ...
        <Slider label={t('quick-return.controls.slider.omega')} id="qr-omega" min={0.2} max={5} step={0.1} value={params.omega} unit="rad/s" onChange={v => onSetParam('omega', v)} />
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-2">
          <Toggle id="qr-velocity"     label={t('quick-return.controls.toggle.velocity')}     checked={visibility.showVelocity}     onChange={() => onToggle('showVelocity')} />
          <Toggle id="qr-acceleration" label={t('quick-return.controls.toggle.acceleration')} checked={visibility.showAcceleration} onChange={() => onToggle('showAcceleration')} />
          <Toggle id="qr-trace"        label={t('quick-return.controls.toggle.trace')}        checked={visibility.showTrace}        onChange={() => onToggle('showTrace')} />
          <Toggle id="qr-guides"       label={t('quick-return.controls.toggle.guides')}       checked={visibility.showGuides}       onChange={() => onToggle('showGuides')} />
        </div>
      </section>
      ...
      <div className="flex gap-2">
        ...
        >
          {paused ? t('quick-return.controls.btn.resume') : t('quick-return.controls.btn.pause')}
        </button>
      </div>
      ...
      <section className="text-[10px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-800 pt-3 flex flex-col gap-1.5">
        <div className="flex justify-between gap-2">
          ...
          <span className="font-mono text-gray-700 dark:text-gray-300">r·sin φ</span>
        </div>
        <div className="flex justify-between gap-2">
          <span>{t('quick-return.controls.info.xP')}</span>
          <span className="font-mono text-gray-700 dark:text-gray-300">L₁·x_B / y_B</span>
        </div>
        <div className="flex justify-between gap-2">
