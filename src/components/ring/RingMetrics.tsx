...
export const RingMetrics = memo(function RingMetrics({ state, params }: Props) {
  ...
  const cards = [
    {
      label: t("ring.metrics.theta"),
      value: normalizeDeg(state.theta).toFixed(1),
      unit: "°",
      danger: false,
    },
    {
      label: t("ring.metrics.v"),
      value: state.v.toFixed(1),
      unit: "u/s",
      danger: false,
    },
    {
      label: t("ring.metrics.n"),
      value: state.N.toFixed(1),
      unit: "u/s²",
      danger: lostContact,
    },
    {
      label: t("ring.metrics.h"),
      value: state.h.toFixed(1),
      unit: "u",
      danger: false,
    },
    {
      label: t("ring.metrics.ke"),
      value: state.KE.toFixed(0),
      unit: "u²/s²",
      danger: false,
    },
    {
      label: t("ring.metrics.pe"),
      value: state.PE.toFixed(0),
      unit: "u²/s²",
      danger: false,
    },
  ];
      ...
      <div className="mt-3 text-[10px] text-gray-500 dark:text-gray-400">
        ...
        <div className="mt-1 flex justify-between">
          <span className="font-mono">
            v₀ = {params.initialSpeed.toFixed(0)} u/s
          </span>
          <span className="font-mono">
            {ratioPct}% {t("ring.metrics.vmin_pct")}
          </span>
        </div>
      </div>
    </div>
