...
interface Props {
  ...
  onSetParam: <K extends keyof RingParams>(key: K, value: RingParams[K]) => void
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-3 bg-white dark:bg-gray-900 flex flex-col gap-4">
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          <Slider label={t('ring.controls.slider.radius')}  id="radius"  min={60} max={130} step={5}  value={params.radius}  unit="u"     onChange={v => onSetParam('radius',  v)} />
          <Slider label={t('ring.controls.slider.gravity')} id="gravity" min={50} max={400} step={10} value={params.gravity} unit="u/s²"  onChange={v => onSetParam('gravity', v)} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-2">
          <Toggle id="showWeight"       label={t('ring.controls.toggle.weight')}     checked={visibility.showWeight}       onChange={() => onToggle('showWeight')} />
          <Toggle id="showNormal"       label={t('ring.controls.toggle.normal')}     checked={visibility.showNormal}       onChange={() => onToggle('showNormal')} />
          <Toggle id="showVelocity"     label={t('ring.controls.toggle.velocity')}   checked={visibility.showVelocity}     onChange={() => onToggle('showVelocity')} />
          <Toggle id="showEnergyBar"    label={t('ring.controls.toggle.energy_bar')} checked={visibility.showEnergyBar}    onChange={() => onToggle('showEnergyBar')} />
          <Toggle id="showTrace"        label={t('ring.controls.toggle.trace')}      checked={visibility.showTrace}        onChange={() => onToggle('showTrace')} />
          <Toggle id="showCriticalMark" label={t('ring.controls.toggle.vmin')}       checked={visibility.showCriticalMark} onChange={() => onToggle('showCriticalMark')} />
        </div>
      </section>
      ...
      <div className="flex gap-2">
        ...
        >
          {paused ? t('ring.controls.btn.resume') : t('ring.controls.btn.pause')}
        </button>
      </div>
      ...
      <section className="text-[10px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-800 pt-3 flex flex-col gap-1.5">
        <div className="flex justify-between gap-2">
          ...
          <span className="font-mono text-gray-700 dark:text-gray-300">θ̈ = −(g/R)·sin θ</span>
        </div>
        <div className="flex justify-between gap-2">
          <span>{t('ring.controls.info.normal')}</span>
          <span className="font-mono text-gray-700 dark:text-gray-300">N = m·(g·cos θ + R·θ̇²)</span>
        </div>
      </section>
    </div>
  )
