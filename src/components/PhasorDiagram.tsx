...
import { COLORS, COLORS_DARK, drawArrow, drawDot, drawLabel, type ColorPalette } from '@/lib/drawing'
...
export const PhasorDiagram = memo(function PhasorDiagram({
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (kind === 'velocity') {
        resX = vx; resY = vy
        sT = vMag; sN = 0; sR = rDot; sO = rThetaDot
        lT = 'vₜ'; lN = 'vₙ'; lR = 'vᵣ'; lO = 'vθ'
        ...
        resX = ax; resY = ay
        sT = aT; sN = aNint; sR = ar; sO = aTheta
        lT = 'aₜ'; lN = 'aₙ'; lR = 'aᵣ'; lO = 'aθ'
      ...
      const maxMag = Math.max(
        ...
        Math.abs(sT), Math.abs(sN), Math.abs(sR), Math.abs(sO),
        minMag,
      )
      const targetFit = halfSize / maxMag
      fitRef.current = fitRef.current === 0 ? targetFit : smoothScale(fitRef.current, targetFit)
      ...
      const dashed = (x1: number, y1: number, x2: number, y2: number, color: string) => {
        ctx!.save()
      ...
      if (kind === 'acceleration') {
        ...
        if (aMag * s > 18) {
          drawLabel(ctx!, `a = ${aMag.toFixed(1)}`, resEnd.x + 10, resEnd.y - 10, resultantColor, true)
        }
      }
      ...
      if (Math.abs(sT) * s > 14) {
        drawLabel(ctx!, `${lT}=${sT.toFixed(1)}`, tEnd.x + 12, tEnd.y - 6, cT, true)
      }
      ...
      if (kind === 'velocity') {
        ...
        if (Math.abs(sN) * s > 14) {
          drawLabel(ctx!, `${lN}=${sN.toFixed(1)}`, nEnd.x + 12, nEnd.y - 6, cN, true)
        }
      }
      ...
      if (Math.abs(sR) * s > 14) {
        drawLabel(ctx!, `${lR}=${sR.toFixed(1)}`, rEnd.x + 12, rEnd.y + 8, cR, true)
      }
      // Transverse / "angular"
      drawArrow(ctx!, cx, cy, oEnd.x, oEnd.y, cO, 2)
      if (Math.abs(sO) * s > 14) {
        drawLabel(ctx!, `${lO}=${sO.toFixed(1)}`, oEnd.x + 12, oEnd.y + 8, cO, true)
      }
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-gray-900">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 px-1 pt-1.5 text-[10px] text-gray-600 dark:text-gray-400">
        <span className="flex items-center gap-1">
          <span className="text-gray-500 dark:text-gray-500">{t('phasor.basis.intrinsic')}:</span>
          <span className="inline-block w-2.5 h-[2px]" style={{ backgroundColor: COLORS.acceleration }} />
          <span className="inline-block w-2.5 h-[2px]" style={{ backgroundColor: COLORS.normalAccel }} />
          <span>{intrinsicLabel}</span>
        </span>
        <span className="flex items-center gap-1">
          <span className="text-gray-500 dark:text-gray-500">{t('phasor.basis.polar')}:</span>
          <span className="inline-block w-2.5 h-[2px]" style={{ backgroundColor: COLORS.radialVelocity }} />
          <span className="inline-block w-2.5 h-[2px]" style={{ backgroundColor: COLORS.transverseVelocity }} />
