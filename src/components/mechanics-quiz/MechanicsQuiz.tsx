...
import { useMechanicsQuiz } from "@/hooks/useMechanicsQuiz";

export function MechanicsQuiz() {
  const { t } = useLanguage();
              ...
              {submitted && (
                ...
                >
                  <span className="font-medium">
                    {isCorrect ? t("mq.correct") : t("mq.incorrect")}:
                  </span>{" "}
      ...
      <div className="flex items-center gap-3">
        ...
        {submitted && (
          <span className="ml-auto text-sm font-semibold text-gray-900 dark:text-gray-100">
            {t("mq.score_label")}: {score}/{items.length}
          </span>
        )}
      </div>
    </div>
